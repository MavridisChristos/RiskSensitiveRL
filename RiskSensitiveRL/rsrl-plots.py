...
for risk_beta in risk_betas:
    ...
    for lr in learning_rates:
        ...
        plt.plot(x,y, label=f'(l={actual_model_var})',color='k',linewidth=2,alpha=0.7, marker='D', markersize = 2)
        ...
        for ll in range(len(testing_data[0])):
            ...
            color = colors[ll]
            plt.plot(x,y, label=f'(l={actual_model_var + model_var[ll]:.1f})',color=color,linewidth=2,alpha=0.5, marker='D', markersize = 2)
            plt.fill(xfill,yfill,
                      alpha=.05, fc=color, ec='None')
            
            print(testing_all_cvar)
            xx = len(training_all) + len(testing_all)
            yy = testing_all_cvar
            plt.plot(xx,yy,color=color,linewidth=2,alpha=0.7,marker='_',markersize=24)
        ...
        plot_file = '../results/'+game+'/'+folder+'/rs-average.png'
