...
names = ['LA00BL-BETA00-NN016/LR0050-Ao00-Ai00.pkl',
...
for n, name in enumerate(names):
    ...
    # save figure
    if True:
        fig.savefig(dfolder+'/'+title+f'-{n}.png', format = 'png')
...
for ll in range(len(testing_all)):  
    ...
    color = colors[ll]
    plt.plot([0,1],[0,1],color=color,linewidth=6,alpha=0.8,label=f'(l={model_var[ll]})')
    
    
    
plt.legend(prop={'size': font_size},bbox_to_anchor=(1.05, 1.02)) 
plt.tight_layout()   
plt.show() 
    
if True:
    fig.savefig(dfolder+'/'+title+'-legend.png', format = 'png')