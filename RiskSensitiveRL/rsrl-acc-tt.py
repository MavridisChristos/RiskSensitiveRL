...
for risk_beta in risk_betas:
    ...
    for lr in learning_rates:
        ...
        if risk_beta>0:
            ...
            color = colors[7]
        plt.plot(x,y, label=r'Training ($\beta$'+f'={risk_beta})',color=color,linewidth=3,alpha=0.7, marker='D', markersize = 5)
        plt.fill(xfill,yfill,
                  alpha=.05, fc='k', ec='None')
        # plt.fill_between([x[0],x[-1]],[goal_max,goal_max],[goal_min,goal_min],alpha=0.03,color='k')
        ...
        for ll in range(len(testing_data[0])):
            ...
            if model_var[ll] in test_var:
                ...
                testing_all = []
                testing_all_std = []
                for i in range(len(testing_data[ll][0])):
                    data = np.array([td[ll][i] for td in testing_data])
                    testing_all.append(data.mean())
                    testing_all_std.append(data.std())
                data = np.array(sum([td[ll] for td in testing_data],[]))
                ...
                color = colors[5]
                plt.plot(x,y, label=f'Testing',color=color,linewidth=3,alpha=0.7, marker='s', markersize = 5)
                plt.fill(xfill,yfill,
                          alpha=.05, fc=color, ec='None')
                
                xx = len(training_all) + len(testing_all)
                yy = testing_all_cvar
                plt.plot(xx,yy,label = f'CVaR={yy}',color=color,linewidth=2,alpha=0.7,marker='.',markersize=5)
                plt.plot(xx,yy,color=color,linewidth=2,alpha=0.7,marker='_',markersize=154)
                plt.text(xx-150,yy+5,'CVaR',color=color, fontsize = 16, fontweight='bold')
            
        # plt.fill_between([x[0],x[-1]],[goal_max,goal_max],[goal_min,goal_min],alpha=0.03,color='r')
        ...
        plt.legend(prop={'size': 18},framealpha=0.51, borderpad=1)

        plot_file = '../results/'+game+'/'+folder+'/acc-tt.png'
