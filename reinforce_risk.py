...
def risk_reinforce(env, policy_estimator, num_episodes=2000, beta=-0.01,
    ...
    for ep in range(num_episodes):
        ...
        while complete == False:
            ...
            if complete:
                ...
                discounted_rewards = discount_rewards_exp(
                    episode_log[:, 2], beta, gamma)
...
def discount_rewards_exp(rewards, beta, gamma):
    discounted_rewards = np.zeros(len(rewards))
    discounted_rewards_exp = np.zeros(len(rewards))
    cumulative_rewards = 0
    for i in reversed(range(0, len(rewards))):
        cumulative_rewards = cumulative_rewards * gamma + rewards[i]
        discounted_rewards[i] = cumulative_rewards
    discounted_rewards_exp = beta*np.exp(beta*discounted_rewards)
    return discounted_rewards_exp