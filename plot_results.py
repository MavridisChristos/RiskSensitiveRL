...
title = 'acrobot-neutral-risk-low-res'

names = ['none000005.pkl',
         'b001x.pkl',
         'b001m.pkl']
...
for n, name in enumerate(names):
    ...
    with open(file, mode='rb') as file:
        training_all,training_avg,training_std,_,_,_ = pickle.load(file)
        ...
        x=(np.arange(len(training_avg))+1)*100
        x=np.insert(x, 0, 1)
...
plt.legend(prop={'size': font_size+4},loc='lower right')
plt.tight_layout()
plt.show()
